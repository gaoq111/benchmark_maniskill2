...
data_dir = "benchmark/sequence/temporal_positioning"
...
def create_task(env, task_id, variations):
    ...
    selected_frames, selected_indices = generate_sequence(env, [directions_1, directions_2], camera_angle, object_A_range, object_B_range)
    
    return selected_frames, directions_1, directions_2, object_A_range, object_B_range, selected_indices
...
def update_index(task_id, variations, directions_1, directions_2, object_A_range, object_B_range, selected_indices):
    ...
    object1, object2, size1, color1, size2, color2, ranges, speed_step, sampling_method, transparency, camera_angle, num_distractors, custom_name1, custom_name2 = variations
    if object == 'custom':
        object_1_description = f"the first {custom_name1}"
        object_2_description = f"the second {custom_name2}"
    else:
        object_1_description = f"the {color1} {object1}"
        object_2_description = f"the {color2} {object2}"

    if object_A_range[0] < object_B_range[0]:
        correct_answer = object_1_description
    elif object_A_range[0] > object_B_range[0]:
    ...
    with jsonlines.open(index_path, mode='a') as writer:
        writer.write({
            ...
            "object_1_size": size1,
            "object_1_color": color1 if object != 'custom' else 'N/A',
            "custom_name1": custom_name1 if object == 'custom' else 'N/A',
            ...
            "object_2_size": size2,
            "object_2_color": color2 if object != 'custom' else 'N/A',
            "custom_name2": custom_name2 if object == 'custom' else 'N/A',
...
for variations in selected_variations:
    frames, directions_1, directions_2, object_A_range, object_B_range, selected_indices = create_task(env, task_id, variations)
